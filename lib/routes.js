...
module.exports = ({ cache, config }) => {
  ...
  const exports = {};
  const { token, url, proxy } = config;
  const shouldProxyPrivateDownload =
    proxy || (token && typeof token === "string" && token.length > 0);
  const shouldProxyDownload = proxy || shouldProxyPrivateDownload;

  // Helpers
  const proxyDownload = (asset, req, res) => {
    const redirect = "manual";
    const headers = { Accept: "application/octet-stream" };
    const options = { headers, redirect };
    const { api_url: rawUrl } = asset;
    const url = shouldProxyPrivateDownload
      ? rawUrl.replace(
  ...
  exports.download = async (req, res) => {
    ...
    if (shouldProxyDownload) {
      proxyDownload(platforms[platform], req, res);
      return;
    }
  ...
  exports.downloadPlatform = async (req, res) => {
    ...
    if (shouldProxyDownload) {
      proxyDownload(latest.platforms[platform], req, res);
      return;
    }
